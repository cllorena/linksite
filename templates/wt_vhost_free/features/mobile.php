...
class HelixUltimateFeatureMobile
{
	...
	public function renderFeature()
	{
		...
		$logo_mobile = $this->params->get('mobile_logo') || $doc->countModules('logo-mobile');
		$logo_desktop = $this->params->get('logo_image') || $this->params->get('logo_text');

		$sitename = Factory::getApplication()->get('sitename');

		$altText = $this->params->get('logo_text', $sitename);
		$image_mobile_width = $this->params->get('mobile_logo_width') ? ' width="' . $this->params->get('mobile_logo_width') . '"' : '';
		$image_mobile_height = $this->params->get('mobile_logo_height') ? ' height="' . $this->params->get('mobile_logo_height') . '"' : '';
		$logo_init = '';

		if ($this->params->get('mobile_logo')) {
			$logo_init .= '<a class="uk-navbar-item' . $remove_logo_padding_cls . ' uk-logo" href="' . Uri::base(true) . '/">';
			$logo_init .= '<img' . $image_mobile_width . $image_mobile_height . ' class="tm-mobile-logo" src="' . $this->params->get('mobile_logo') . '" alt="' . $altText . '" />';
			$logo_init .= '</a>';
		} elseif ($logo_desktop) {
			$logo_init .= '<a class="uk-navbar-item uk-logo" href="' . Uri::base(true) . '/">';
			if ($this->params->get('logo_image')) {
				...
				$logo_init .= '<img' . $logo_width . $logo_height . ' class="tm-logo" src="' . $this->params->get('logo_image') . '" alt="' . $altText . '" />';
				
			} else {
				$logo_init .= $this->params->get('logo_text');
			}
			$logo_init .= '</a>';
		}
		...
		if ($mobile_logo_pos == 'left' || $mobile_toggle == 'left') {
			...
			if ($mobile_logo_pos == 'left' && ($logo_mobile || $logo_desktop)) {
				$output .= $logo_init;
				if ($doc->countModules('logo-mobile')) {
		...
		if ($mobile_logo_pos == 'center' && ($logo_mobile || $logo_desktop)) {
		...
		if ($mobile_logo_pos == 'right' || $mobile_toggle == 'right') {
			...
			if ($mobile_logo_pos == 'right' && ($logo_mobile || $logo_desktop) ) {
				$output .= $logo_init;
				if ($doc->countModules('logo-mobile')) {
					$output .= '<jdoc:include type="modules" name="logo-mobile" style="warp_xhtml" />';
				}
		...
		if ($mobile_animations == 'dropdown') {
			...
			if ($doc->countModules('offcanvas')) {
				$output  .= '<jdoc:include type="modules" name="offcanvas" style="grid_stack" />';
			} else {
				$output .= '<p class="uk-alert uk-alert-primary">';
				$output .= JText::_('HU_NO_MODULE_MOBILE_OFFCANVAS');
				$output .= '</p>';
			}
			$output  .= '</div>';
