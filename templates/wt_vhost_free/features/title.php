...
class HelixUltimateFeatureTitle
{
	...
	public function renderFeature()
	{
		...
		$header_height_init = (substr($header_height, 0, -2) . '') / 2;
		...
		if($menuitem)
		{
			...
			if($params->get('helixultimate_enable_page_title', 0))
			{
				...
				if($page_title_bg_image)
				{
					...
					$style .= 'background-image: url(' . \JURI::root(true) . '/' . $theme_page_title_background_image . ');';
				}
				...
				if($header_style == 'style-18') {
					$output .= '<div class="tm-header-placeholder uk-margin-remove-adjacent uk-visible@m" style="height:'.$header_height_init.'px;"></div>';
				}
