...
class HelixUltimateFeatureContact
{
	...
	public function renderFeature()
	{
		...
		if($conditions)
		{
			...
			{
				$output .= '<div class="sp-contact-phone"><a href="tel:' . str_replace(array(')','(',' ','-'),array('','','',''), $this->params->get('contact_phone')) . '"><span class="uk-margin-small-right fas fa-phone-alt" aria-hidden="true"></span>' . $this->params->get('contact_phone') . '</a></div>';
			}

			if($this->params->get('contact_email'))
			{
				$output .= '<div class="sp-contact-email"><a href="mailto:'. $this->params->get('contact_email') .'"><span class="uk-margin-small-right far fa-envelope" aria-hidden="true"></span>' . $this->params->get('contact_email') . '</a></div>';
			}
