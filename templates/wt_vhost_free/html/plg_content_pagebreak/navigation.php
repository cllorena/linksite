...
<div class="uk-grid-small uk-flex-middle uk-text-default" uk-grid>
	<div>
		<a class="uk-button uk-button-secondary" href="<?= $links['next'] ?>" aria-label="<?= $ariaLabel ?>" rel="next"><?= JText::_('TPL_HU_NEXT_PAGE') ?></a>
	</div>

	<?php if ($this->list[$page + 2]->title != JText::sprintf('JLIB_HTML_PAGE_CURRENT', $page + 2)) : ?>
	<div>
		<?= $title ?>
	</div>
	<?php endif ?>
...
<div class="uk-grid-small uk-flex-middle uk-child-width-auto uk-text-default uk-margin" uk-grid>
	<div>
		<?= JText::sprintf('JLIB_HTML_PAGE_CURRENT', '') ?>
	</div>
	<div>
		...
		<ul class="uk-pagination">
			...
			<li class="<?= $item->liClass ?>">
				...
				<?php else : ?>
					<a href="<?= $item->link ?>" class="<?= $item->class ?>"><?= (JText::sprintf('PLG_CONTENT_PAGEBREAK_ALL_PAGES') == $item->title) ? $item->title : $index ?></a>
				<?php endif ?>
			</li>
			<?php endforeach ?>
		</ul>
