...
$result = "select * FROM #__spidercatalog_params";
$db->setQuery($result);
$parameters=$db->loadObjectList();
if(count($parameters) == 134){
$query="INSERT INTO `#__spidercatalog_params` (`id`, `name`, `title`, `description`, `value`) VALUES
(166, 'param_category_title_background', 'Param category title background', 'Param category title background', '004372')";
$db->setQuery($query);
$db->Query();
}
