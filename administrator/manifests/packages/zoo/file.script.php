...
class pkg_zooInstallerScript extends ZooInstallerScript {
    ...
    public function postflight($type, $parent, $results) {
        ...
        if ($db->getNumRows() == 0) {
            ...
            // search menutypes, use 'mainmenuright' when present
            $menutypes = $db->setQuery("SELECT `id`, `menutype` FROM `#__menu_types`")->loadAssocList('id', 'menutype');
            $menutype = in_array('mainmenuright', $menutypes) ? 'mainmenuright' : 'mainmenu';
            ...
            if ($zoo_menu_id && $component_id) {
                ...
                // update ZOO menu
                $db->setQuery("UPDATE `#__menu` SET `component_id` = {$component_id}, `menutype` = '{$menutype}' WHERE `alias` LIKE '%-zoo' AND `title` NOT LIKE 'com_zoo'")->execute();

                // update ZOO submenus
                $db->setQuery("UPDATE `#__menu` SET `parent_id` = {$zoo_menu_id}, `menutype` = '{$menutype}' WHERE `alias` LIKE '%-zoo' AND `id` <> {$zoo_menu_id} AND `title` NOT LIKE 'com_zoo'")->execute();
            ...
            if (property_exists(json_decode($tmplParams), 'yootheme')) {
    ...
    public function uninstall($parent) {
        ...
        // remove menu items
        $db->setQuery("DELETE FROM `#__menu` WHERE `menutype` = '".$menutype."' AND `alias` LIKE '%-zoo'")->execute();
